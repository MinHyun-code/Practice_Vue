    ...
    <div class="content">
      ...
      <div v-if="this.$store.state.modalBoardInsert_tf">
        <div style="width: 98%; margin: 2%;">
          <h2><input type="text" :value="boardTitle" @input="titleChange($event)"/></h2>
          
          <textarea :value="boardContent" @input="textChange($event)" style="width: 47%; display:flex; min-height:50vh; float:left;"></textarea>
          <Markdown :source="boardContent" style="width: 47%; float:right; overflow:auto; height:50vh; margin: 2%;"/>
        </div>
      </div>
			<a class="btn btn-sm btn-outline-secondary"  type="submit" v-on:click="boardInsert()">등록</a>
    </div>
  </template>
  ...
  methods: {    
    ...
    boardInsert() {
      const form = new FormData();
				...
				this.$axios.post("/api/board/insert", form)
				.then((res)=>{
          toastr.success(res);
